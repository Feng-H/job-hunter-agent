...
* **自动同步**：正常浏览 Boss直聘/猎聘/智联时自动采集——列表页批量快筛（卡片自带公司名，无需进详情页），**点开岗位详情页自动深抓全量 JD**；
* **全量 JD 富化重评**：详情页抓取的全量 JD（含福利标签/真实薪资/发布时间）会自动合并进已入库岗位并按最新规则重新初筛，评分随素材增加而提升；
* **智联新版全适配**：优先读取页面结构化数据（`__INITIAL_STATE__`），未登录也能拿到真实薪资与详情 URL；列表流/推荐流/详情页三级降级提取；
* **三层防重复**：客户端 URL 归一指纹去重（重复岗位零请求）+ 服务端 MD5 指纹兜底（同一岗位从列表/详情/多次刷到均不重复入库，详情抓取豁免去重以富化已有记录）；
...
* **两级判定语义**：触发硬红线（非双休/地点不符/薪资低于底线/黑名单/僵尸岗/通勤超限）才淘汰；零红线但契合度评分未达推送门槛的岗位**留在看板待人工复核**（评分门槛只管推送，不掩埋岗位）；
* **规则变更可回溯**：设置页一键「按最新规则重筛全部历史岗位」，误配置后可批量抢救；
...
* **PDF 简历云端直传**：云端自动切换 unpdf 纯 JS 提取引擎（pdf.js 内核，支持中文），无需任何系统二进制；本地优先使用 pdftotext（还原质量最佳）。
...
```bash
npm test    # 三套回归测试：事故回归（19 项）+ 账号持久化（12 项）+ Demo 隔离/雷达/去重
```

测试套件覆盖历史上出现过的每一类故障：账号静默丢失、多实例会话覆盖、偏好设置丢字段、过滤器配置源错误、硬红线/软门槛语义、JD 富化、前端接口契约（静态扫描前端 28 个调用与后端路由一一对应）。**任何改动推送前请先跑 `npm test`。**
...
- **🛡️ 零风控采集**：扩展对招聘平台 100% DOM 只读（无任何模拟点击/请求），出站请求仅发往自有服务器；云端从不触碰招聘平台；
- **💾 云端数据一致性**：账号库读-改-写防多实例覆盖；关键批处理（采集/重筛/看板读写）写前强制重载最新数据；KV 读写 429/网络错误自动重试；
